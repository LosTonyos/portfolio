...
function extendMaterial(BaseMaterial: typeof THREE.MeshStandardMaterial, cfg: Cfg) {
  ...
  if (defaults.color) (uniforms.diffuse as { value: THREE.Color }).value = defaults.color;
  if ("roughness" in defaults) (uniforms.roughness as { value: number }).value = defaults.roughness as number;
  if ("metalness" in defaults) (uniforms.metalness as { value: number }).value = defaults.metalness as number;
...
type MergedPlanesProps = { material: THREE.ShaderMaterial; width: number; count: number; height: number };
